...
app.get('/api/competitions',auth,async(req,res)=>res.json(await q('SELECT * FROM competitions WHERE date>=CURRENT_DATE ORDER BY date,time')));
app.post('/api/competitions',auth,admin,async(req,res)=>{let x=req.body;res.json(await q('INSERT INTO competitions(title,date,time,venue,description,document_url) VALUES($1,$2,$3,$4,$5,$6) RETURNING *',[x.title,x.date,x.time||null,x.venue||'',x.description||'',x.document_url||'']))});
...
app.post('/api/announcements',auth,admin,async(req,res)=>{
  try{
    let x=req.body;
    let row=(await q('INSERT INTO announcements(title,text,priority,pinned) VALUES($1,$2,$3,$4) RETURNING *',[x.title||'',x.text||'',x.priority||'normal',!!x.pinned]))[0];
    res.json(row);
...
app.put('/api/announcements/:id',auth,admin,async(req,res)=>{
  try{
    let x=req.body;
    let row=(await q('UPDATE announcements SET title=$1,text=$2,priority=$3,pinned=$4 WHERE id=$5 RETURNING *',[x.title||'',x.text||'',x.priority||'normal',!!x.pinned,req.params.id]))[0];
